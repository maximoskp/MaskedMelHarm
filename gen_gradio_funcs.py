...
def generate_files_with_base2(
    ...
    # add piano sound
    gen_score  = ensure_instruments(gen_score)    
    ...
    # add piano sound
    real_score = ensure_instruments(real_score)
...
def ensure_instruments(score):
    """
    Guarantee every Part in `score` starts with an Instrument object so that
    MuseScore doesn’t complain 'no instrument found for part …'.
    """
    for idx, part in enumerate(score.parts, start=1):
        # If no Instrument of any kind is already present → add one
        if not part.getElementsByClass(m21.instrument.Instrument):
            instr = m21.instrument.Piano()               # brand-new object
            instr.partName = "Piano"
            part.insert(0, instr)

        # Give the Part a stable ID MuseScore can reference
        if not part.id:
            part.id = f"P{idx}"
